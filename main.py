...
def _publish_all(state: PipelineState, output_dir: Path):
    ...
    try:
        ...
        drive_title = f"{today_str} {state.topic}"
        console.print("[cyan]Uploading to Google Drive...[/cyan]")
        doc_url = upload_docx_to_drive(docx_path, drive_title)
    ...
    try:
        ...
        col_c = ws.col_values(3)  # Column C = Blog name
        today = date.today().strftime("%-m/%-d/%Y")  # Match spreadsheet date format
        found_row = None
        for row_idx, val in enumerate(col_c):
            if state.topic.lower() in val.lower():
                found_row = row_idx + 1  # 1-indexed
                break

        if found_row:
            # Read the full row
            row_data = ws.row_values(found_row)
            # Pad to at least 3 columns
            while len(row_data) < 3:
                row_data.append("")
            # Update: column A = status note, column B = today's date, column C = blog name with marker
            row_data[0] = f"CLAUDE CODE GENERATED"
            row_data[1] = today
            if "CLAUDE CODE" not in row_data[2]:
                row_data[2] = f"{row_data[2]} -- GENERATED BY CLAUDE CODE"

            # Delete the original row
            ws.delete_rows(found_row)
            # Insert updated row at row 2 (just below frozen header)
            ws.insert_row(row_data, index=2)
            console.print(f"[bold green]Spreadsheet updated:[/bold green] Moved to top, publish date set to {today}")
        else:
